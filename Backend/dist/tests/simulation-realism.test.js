...
describe("Simulation Realism Tests", () => {
    ...
    describe("Battery Realism", () => {
        it("all batteries use 588 kWh capacity", async () => {
            ...
            const batteryAfter = await (0, connection_1.getQuery)("SELECT capacityKwh FROM batteries WHERE id = ?;", [battery.body.battery.id]);
        ...
        it("battery health degrades slowly over time", async () => {
            ...
            const batteryAfter = await (0, connection_1.getQuery)("SELECT health, cycleCount FROM batteries WHERE id = ?;", [battery.body.battery.id]);
        ...
        it("battery cycle count increments on swap", async () => {
            ...
            const outgoingBattery = await (0, supertest_1.default)(app_1.default)
                .post("/batteries")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
                capacityKwh: 588,
            ...
            const incomingBattery = await (0, supertest_1.default)(app_1.default)
                .post("/batteries")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
                capacityKwh: 588,
            ...
            await (0, orchestrator_1.runSimulationCycle)();
            const batteryAfter = await (0, connection_1.getQuery)("SELECT cycleCount, status FROM batteries WHERE id = ?;", [outgoingBattery.body.battery.id]);
        ...
        it("battery temperature changes during charging", async () => {
            ...
            const batteryAfter = await (0, connection_1.getQuery)("SELECT temperature, status FROM batteries WHERE id = ?;", [battery.body.battery.id]);
        ...
        it("battery status transitions correctly", async () => {
            ...
            await (0, orchestrator_1.runSimulationCycle)();
            const batteryAfter = await (0, connection_1.getQuery)("SELECT status, soc FROM batteries WHERE id = ?;", [battery.body.battery.id]);
    ...
    describe("Truck Realism", () => {
        it("truck location updates continuously during transit", async () => {
            ...
            const fleetId = fleet.body.fleet.id;
            const driver = await (0, supertest_1.default)(app_1.default)
                .post("/drivers")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
                name: "Test Driver",
            ...
            const truckId = truck.body.truck.id;
            const initialLocation = await (0, connection_1.getQuery)("SELECT locationLat, locationLng FROM trucks WHERE id = ?;", [truckId]);
            // Run simulation to start movement
            await (0, orchestrator_1.runSimulationCycle)();
            const locationAfter = await (0, connection_1.getQuery)("SELECT locationLat, locationLng, status, currentStationId FROM trucks WHERE id = ?;", [truckId]);
        ...
        it("truck currentStationId updates on arrival/departure", async () => {
            ...
            // Run simulation to start movement
            await (0, orchestrator_1.runSimulationCycle)();
            const truckAfter = await (0, connection_1.getQuery)("SELECT currentStationId, status FROM trucks WHERE id = ?;", [truckId]);
        ...
        it("refrigerated trucks consume more energy", async () => {
            ...
            const driverId = driver.body.driver.id;
            const standardTruck = await (0, supertest_1.default)(app_1.default)
                .post("/trucks")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
                plateNumber: "ET-1001",
            ...
            const refrigeratedTruck = await (0, supertest_1.default)(app_1.default)
                .post("/trucks")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
                plateNumber: "ET-2001",
            ...
            const standardBattery = await (0, supertest_1.default)(app_1.default)
                .post("/batteries")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
                capacityKwh: 588,
            ...
            const refrigeratedBattery = await (0, supertest_1.default)(app_1.default)
                .post("/batteries")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
                capacityKwh: 588,
            ...
            const standardSoc = await (0, connection_1.getQuery)("SELECT soc FROM batteries WHERE id = ?;", [standardBattery.body.battery.id]);
            const refrigeratedSoc = await (0, connection_1.getQuery)("SELECT soc FROM batteries WHERE id = ?;", [refrigeratedBattery.body.battery.id]);
        ...
        it("trucks can enter maintenance occasionally", async () => {
            ...
            for (let i = 0; i < 200; i++) {
                await (0, orchestrator_1.runSimulationCycle)();
                const truckStatus = await (0, connection_1.getQuery)("SELECT status FROM trucks WHERE id = ?;", [truckId]);
    ...
    describe("Queue Realism", () => {
        it("queue records are created when trucks need swaps", async () => {
            ...
            const truckId = truck.body.truck.id;
            // Set truck location
            await (0, connection_1.runQuery)("UPDATE trucks SET locationLat = 9.0, locationLng = 40.0 WHERE id = ?;", [truckId]);
            // Run simulation to trigger queue management
            await (0, orchestrator_1.runSimulationCycle)();
            const queueEntry = await (0, connection_1.getQuery)("SELECT id, truckId, stationId, status FROM swap_queue WHERE truckId = ?;", [truckId]);
        ...
        it("queue is station-specific", async () => {
            ...
            const driverId = driver.body.driver.id;
            const truck1 = await (0, supertest_1.default)(app_1.default)
                .post("/trucks")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
                plateNumber: "ET-1001",
            ...
            const truck2 = await (0, supertest_1.default)(app_1.default)
                .post("/trucks")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
                plateNumber: "ET-1002",
            ...
            await (0, connection_1.runQuery)("UPDATE trucks SET locationLat = 8.98, locationLng = 38.76 WHERE id = ?;", [truck1Id]);
            await (0, connection_1.runQuery)("UPDATE trucks SET locationLat = 8.54, locationLng = 39.27 WHERE id = ?;", [truck2Id]);
            // Run simulation
            await (0, orchestrator_1.runSimulationCycle)();
            const queue1 = await (0, connection_1.allQuery)("SELECT truckId, stationId FROM swap_queue WHERE truckId = ?;", [truck1Id]);
            const queue2 = await (0, connection_1.allQuery)("SELECT truckId, stationId FROM swap_queue WHERE truckId = ?;", [truck2Id]);
        ...
        it("queue ordering is deterministic (by distance)", async () => {
            ...
            await (0, connection_1.runQuery)("UPDATE trucks SET locationLat = 8.99, locationLng = 38.76 WHERE id = ?;", [truck1Id]);
            await (0, connection_1.runQuery)("UPDATE trucks SET locationLat = 9.5, locationLng = 40.0 WHERE id = ?;", [truck2Id]);
            // Run simulation
            await (0, orchestrator_1.runSimulationCycle)();
            const queueEntries = await (0, connection_1.allQuery)("SELECT truckId, distanceKm FROM swap_queue WHERE stationId = ? ORDER BY distanceKm ASC, bookedAt ASC;", [stationId]);
    ...
    describe("Charging Realism", () => {
        it("charging respects configured charging windows", async () => {
            ...
            await (0, connection_1.runQuery)("INSERT OR REPLACE INTO charging_window_config (id, startHour, endHour) VALUES (1, 22, 6);");
            const station = await (0, supertest_1.default)(app_1.default)
                .post("/stations")
                .set("Authorization", `Bearer ${adminToken}`)
                .send({
            ...
            // Run simulation
            await (0, orchestrator_1.runSimulationCycle)();
            const batteryAfter = await (0, connection_1.getQuery)("SELECT status FROM batteries WHERE id = ?;", [battery.body.battery.id]);
        ...
        it("charging sessions track current SOC and target SOC", async () => {
            ...
            // Run simulation to start charging
            await (0, orchestrator_1.runSimulationCycle)();
            const session = await (0, connection_1.getQuery)("SELECT startSoc, currentSoc, targetSoc, status FROM charging_sessions WHERE batteryId = ? AND status = 'ACTIVE';", [battery.body.battery.id]);
            if (session) {
        ...
        it("energy added is computed from SOC progression", async () => {
            ...
            const session = await (0, connection_1.getQuery)("SELECT energyAddedKwh, currentSoc FROM charging_sessions WHERE batteryId = ? AND status = 'ACTIVE';", [battery.body.battery.id]);
            if (session) {
        ...
        it("station charger capacity constraints are respected", async () => {
            ...
            // Run simulation
            await (0, orchestrator_1.runSimulationCycle)();
            const activeSessions = await (0, connection_1.allQuery)("SELECT id FROM charging_sessions WHERE stationId = ? AND status = 'ACTIVE';", [stationId]);
    ...
    describe("Incidents and Faults", () => {
        it("generates queue congestion events when threshold exceeded", async () => {
            ...
            // Run simulation
            await (0, orchestrator_1.runSimulationCycle)();
            const incidents = await (0, connection_1.allQuery)("SELECT type, severity, status FROM station_incidents WHERE stationId = ? AND type = 'QUEUE_CONGESTION';", [stationId]);
        ...
        it("generates occasional charger faults", async () => {
            ...
            for (let i = 0; i < 100; i++) {
                await (0, orchestrator_1.runSimulationCycle)();
                const faults = await (0, connection_1.allQuery)("SELECT id FROM charger_faults WHERE stationId = ? AND status = 'OPEN';", [stationId]);
                if (faults.length > 0) {
                    // Fault generated

...
describe("Accounting Tests", () => {
    ...
    describe("Standard Swap Math", () => {
        it("calculates receipt correctly for standard swap", async () => {
            // Set tariff
            await (0, connection_1.runQuery)("INSERT OR REPLACE INTO tariff_config (id, eeuRatePerKwh, a2ServiceRatePerKwh, vatPercent) VALUES (1, 10, 10, 15);");
         ...
         VALUES (?, ?, ?, ?, 30, ?, ?);`, [truckId, stationId, incomingBatteryId, outgoingBatteryId, energyDeliveredKwh, new Date().toISOString()]);
            // Run simulation to create receipt
            await (0, orchestrator_1.runSimulationCycle)();
            // Get receipt
            const receipt = await (0, connection_1.getQuery)("SELECT energyKwh, energyCharge, serviceCharge, vat, total, eeuShare, a2Share FROM receipts WHERE swapId = ?;", [
                swapResult.lastID,
            ]);
    ...
    describe("VAT Correctness", () => {
        it("calculates VAT correctly as percentage of subtotal", async () => {
            ...
            const swapResult = await (0, connection_1.runQuery)(`INSERT INTO swap_transactions 
         (truckId, stationId, incomingBatteryId, outgoingBatteryId, arrivalSoc, energyDeliveredKwh, timestamp)
         VALUES (?, ?, ?, ?, 30, ?, ?);`, [truckId, stationId, incomingBatteryId, outgoingBatteryId, energyKwh, new Date().toISOString()]);
            await (0, orchestrator_1.runSimulationCycle)();
            const receipt = await (0, connection_1.getQuery)("SELECT vat, energyCharge, serviceCharge FROM receipts WHERE swapId = ?;", [swapResult.lastID]);
    ...
    describe("Share Split Correctness", () => {
        it("splits VAT 50/50 between EEU and A2", async () => {
         ...
         VALUES (?, ?, ?, ?, 30, ?, ?);`, [truckId, stationId, incomingBatteryId, outgoingBatteryId, energyKwh, new Date().toISOString()]);
            await (0, orchestrator_1.runSimulationCycle)();
            const receipt = await (0, connection_1.getQuery)("SELECT energyCharge, serviceCharge, vat, eeuShare, a2Share, total FROM receipts WHERE swapId = ?;", [
                swapResult.lastID,
            ]);
    ...
    describe("Station Daily/Monthly Summaries", () => {
        it("calculates station revenue correctly for daily timeframe", async () => {
         ...
         VALUES (?, ?, ?, ?, 30, ?, ?);`, [truckId, stationId, incomingBatteryId, outgoingBatteryId, energyDeliveredKwh, new Date().toISOString()]);
            await (0, orchestrator_1.runSimulationCycle)();
            // Get station summary
            const summaryResponse = await (0, supertest_1.default)(app_1.default)
                .get(`/billing/summary/stations?timeframe=daily`)
                .set("Authorization", `Bearer ${adminToken}`);
            expect(summaryResponse.status).toBe(200);
            const stationSummary = summaryResponse.body.revenueByStation.find((s) => s.stationId === stationId);
    ...
    describe("Fleet Cost Correctness", () => {
        it("calculates fleet energy cost correctly", async () => {
         ...
         VALUES (?, ?, ?, ?, 30, ?, ?);`, [truckId, stationId, incomingBatteryId, outgoingBatteryId, energyDeliveredKwh, new Date().toISOString()]);
            ...
            expect(summaryResponse.status).toBe(200);
            const fleetSummary = summaryResponse.body.revenueByFleet.find((f) => f.fleetId === fleetId);
    ...
    describe("A2 and EEU Totals Reconciliation", () => {
        it("A2 totals reconcile against receipts", async () => {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                await (0, connection_1.runQuery)(`INSERT INTO swap_transactions 
           (truckId, stationId, incomingBatteryId, outgoingBatteryId, arrivalSoc, energyDeliveredKwh, timestamp)
           VALUES (?, ?, ?, ?, 30, ?, ?);`, [truckId, stationId, batteries[3 + i], batteries[i], energyKwh, new Date().toISOString()]);
            }
            await (0, orchestrator_1.runSimulationCycle)();
            // Get A2 summary

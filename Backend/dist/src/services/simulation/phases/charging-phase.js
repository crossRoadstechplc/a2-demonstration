...
exports.runChargingPhase = runChargingPhase;
...
async function runChargingPhase(context) {
    const { timestamp } = context;
    const now = new Date(timestamp);
    const hour = now.getHours();
    // Get charging window config
    const window = await (0, connection_1.getQuery)(`SELECT startHour, endHour FROM charging_window_config WHERE id = 1;`) ?? { startHour: 22, endHour: 6 };
    ...
    const batteriesByStation = new Map();
    ...
    const activeSessions = await (0, connection_1.allQuery)("SELECT id, batteryId, stationId, startSoc, currentSoc, targetSoc, energyAddedKwh FROM charging_sessions WHERE status = 'ACTIVE';");
    ...
    for (const session of activeSessions) {
        ...
        const energyAddedThisCycle = (0, utils_1.round2)(CHARGER_OUTPUT_KW * cycleTimeHours * CHARGER_EFFICIENCY * chargeRateMultiplier * chargingRateMultiplierScenario);
        // Convert energy to SOC increase
        const socIncrease = (0, utils_1.round2)((energyAddedThisCycle / STANDARD_BATTERY_CAPACITY_KWH) * 100);
        const nextSoc = Math.min(session.targetSoc, (0, utils_1.round2)(battery.soc + socIncrease));
        const actualEnergyAdded = (0, utils_1.round2)((STANDARD_BATTERY_CAPACITY_KWH * (nextSoc - battery.soc)) / 100);
        const totalAddedKwh = (0, utils_1.round2)(session.energyAddedKwh + actualEnergyAdded);
        // Update battery SOC
      ...
      `, [timestamp, nextSoc, totalAddedKwh, session.id]);
            ...
            await (0, connection_1.runQuery)("UPDATE charging_sessions SET currentSoc = ?, energyAddedKwh = ? WHERE id = ?;", [nextSoc, totalAddedKwh, session.id]);
        }
    }
}
async function getStationChargerCapacities() {
    ...
    for (const station of stations) {
        const batteryCount = await (0, connection_1.getQuery)("SELECT COUNT(*) as count FROM batteries WHERE stationId = ?;", [station.id]);

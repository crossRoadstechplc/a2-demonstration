...
async function ensureInitialRealisticData(stationIds, timestamp) {
    ...
    if ((swapsToday?.count ?? 0) === 0) {
        const trucks = await (0, connection_1.allQuery)("SELECT id, currentStationId FROM trucks ORDER BY id ASC LIMIT 1200;");
        // Create 800-1000 swaps across different stations for today
        const swapCount = Math.min(800 + (0, utils_1.randomInt)(200), trucks.length);

...
async function runBootstrapPhase(context) {
...
async function ensureTruckLocationsInitialized(stations) {
    if (!stations.length)
        return;
    const trucksMissingLocation = await (0, connection_1.allQuery)(`
  ...
  `);
    for (const truck of trucksMissingLocation) {
        ...
        const coordinate = (0, utils_1.getStationCoordinate)(station);
        await (0, connection_1.runQuery)("UPDATE trucks SET currentStationId = COALESCE(currentStationId, ?), locationLat = ?, locationLng = ? WHERE id = ?;", [station.id, coordinate.lat, coordinate.lng, truck.id]);
    }
}
async function ensureSimulationBatteries(stationIds) {
    ...
    const stations = await (0, connection_1.allQuery)("SELECT id, capacity FROM stations ORDER BY id ASC;");
    const stationCapacityMap = new Map(stations.map((s) => [s.id, s.capacity]));
    // Get trucks for IN_TRUCK battery assignment
    const trucks = await (0, connection_1.allQuery)("SELECT id FROM trucks ORDER BY id ASC;");
    let truckIndex = 0;
    for (const stationId of stationIds) {
        ...
        let totalBatteries;
        if (capacity <= 18) {
            totalBatteries = 150 + (0, utils_1.randomInt)(50); // 150-200
        }
        else if (capacity <= 25) {
            totalBatteries = 250 + (0, utils_1.randomInt)(100); // 250-350
        }
        else {
            totalBatteries = 400 + (0, utils_1.randomInt)(200); // 400-600
        }
        ...
        for (let i = 0; i < readyCount; i++) {
            const soc = Math.max(25, 80 + (0, utils_1.randomInt)(15)); // 80-95%
      ...
      `, [soc, stationId]);
        ...
        for (let i = 0; i < chargingCount; i++) {
            ...
            if (progress < 0.2) {
                // 20% near completion
                soc = Math.max(25, 85 + (0, utils_1.randomInt)(10)); // 85-95%
            }
            else if (progress < 0.6) {
                // 40% mid-cycle
                soc = Math.max(25, 50 + (0, utils_1.randomInt)(30)); // 50-80%
            }
            else {
                // 40% newly charging
                soc = Math.max(25, 25 + (0, utils_1.randomInt)(25)); // 25-50%
            }
            await (0, connection_1.runQuery)(`
        INSERT INTO batteries
        (capacityKwh, soc, health, cycleCount, temperature, status, stationId, truckId)
      ...
      `, [soc, stationId]);
        ...
        for (let i = 0; i < maintenanceCount; i++) {
            const soc = Math.max(25, 50 + (0, utils_1.randomInt)(30)); // 50-80%
            await (0, connection_1.runQuery)(`
        INSERT INTO batteries
        (capacityKwh, soc, health, cycleCount, temperature, status, stationId, truckId)
      ...
      `, [soc, stationId]);
        ...
        for (let i = 0; i < inTruckCount; i++) {
            if (truckIndex >= trucks.length)
                break; // Stop if we run out of trucks
            const truck = trucks[truckIndex];
            truckIndex++;
            // Create new IN_TRUCK battery (never convert from READY)
            const soc = Math.max(25, 60 + (0, utils_1.randomInt)(30)); // 60-90%
      ...
      `, [soc, truck.id]);
            ...
            await (0, connection_1.runQuery)(`UPDATE trucks SET batteryId = ?, currentSoc = ? WHERE id = ?;`, [String(result.lastID), soc, truck.id]);
        }
    }
}
async function seedInitialChargingSessions(timestamp) {
    // Get all CHARGING batteries that don't have active sessions
    const chargingBatteries = await (0, connection_1.allQuery)(`
    ...
    for (const battery of chargingBatteries) {
        ...
        if (currentSoc >= 85) {
            // Near completion
            startSoc = Math.max(25, 20 + (0, utils_1.randomInt)(10)); // 20-30%
            hoursAgo = 4 + (0, utils_1.randomInt)(2); // 4-6 hours
        }
        else if (currentSoc >= 50) {
            // Mid-cycle
            startSoc = Math.max(25, 15 + (0, utils_1.randomInt)(10)); // 15-25%
            hoursAgo = 2 + (0, utils_1.randomInt)(2); // 2-4 hours
        }
        else {
            // Newly charging
            startSoc = Math.max(25, 20 + (0, utils_1.randomInt)(5)); // 20-25%
            hoursAgo = 1 + (0, utils_1.randomInt)(1); // 1-2 hours
        }
        // Calculate energy added based on SOC difference
        const socDelta = currentSoc - startSoc;
        const energyAddedKwh = (0, utils_1.round2)((STANDARD_BATTERY_CAPACITY_KWH * socDelta) / 100);
...
async function ensureDriverAssignments() {
    const unassignedDrivers = await (0, connection_1.allQuery)("SELECT id, fleetId, assignedTruckId FROM drivers WHERE assignedTruckId IS NULL ORDER BY id ASC;");
    ...
    for (const driver of driversToAssign) {
        ...
        await (0, connection_1.runQuery)("UPDATE trucks SET assignedDriverId = ?, availability = 'ACTIVE', status = 'IN_TRANSIT' WHERE id = ?;", [
            driver.id,
            truck.id,
        ]);
    }
}
async function ensureInitialRealisticData(stationIds, timestamp) {
    // Check if we already have swap data for today
    const swapsToday = await (0, connection_1.getQuery)(`SELECT COUNT(*) as count FROM swap_transactions 
    ...
    if ((swapsToday?.count ?? 0) === 0) {
        const trucks = await (0, connection_1.allQuery)("SELECT id, currentStationId FROM trucks ORDER BY id ASC LIMIT 2000;");
        // Create 1200-1500 swaps across different stations for today
        const swapCount = Math.min(1200 + (0, utils_1.randomInt)(300), trucks.length);
        for (let i = 0; i < swapCount; i++) {
            const truck = trucks[i];
            const stationId = truck.currentStationId ?? stationIds[truck.id % stationIds.length];
            // Get real batteries for this station
            const stationBatteries = await (0, connection_1.allQuery)("SELECT id FROM batteries WHERE stationId = ? ORDER BY id ASC LIMIT 10;", [stationId]);
            if (stationBatteries.length < 2)
            ...
            const energyKwh = 200 + (0, utils_1.randomInt)(200); // 200-400 kWh
            const hoursAgo = (0, utils_1.randomInt)(12); // Swaps from last 12 hours
            ...
            const arrivalSoc = Math.max(25, 30 + (0, utils_1.randomInt)(20));
            const swapInsert = await (0, connection_1.runQuery)(`INSERT INTO swap_transactions 
         (truckId, stationId, incomingBatteryId, outgoingBatteryId, arrivalSoc, energyDeliveredKwh, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?);`, [truck.id, stationId, incomingBatteryId, outgoingBatteryId, arrivalSoc, energyKwh, swapTime]);
            ...
            await (0, connection_1.runQuery)(`INSERT INTO receipts
         (swapId, energyKwh, energyCharge, serviceCharge, vat, total, eeuShare, a2Share, paymentMethod, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`, [swapInsert.lastID, energyKwh, energyCharge, serviceCharge, vat, total, eeuShare, a2Share, paymentMethod, swapTime]);
        }
        // Create historical completed charging sessions (20-30 completed today)
        const completedSessionCount = 20 + (0, utils_1.randomInt)(10);
        const chargingBatteries = await (0, connection_1.allQuery)(`SELECT id, stationId, soc FROM batteries 
       WHERE status = 'CHARGING' AND stationId IS NOT NULL 
       ORDER BY id ASC LIMIT ?;`, [completedSessionCount]);
        const STANDARD_BATTERY_CAPACITY_KWH = 588;
        for (let i = 0; i < Math.min(completedSessionCount, chargingBatteries.length); i++) {
            ...
            const startHoursAgo = 3 + (0, utils_1.randomInt)(5); // 3-8 hours
            const endHoursAgo = 1 + (0, utils_1.randomInt)(2); // 1-3 hours
            ...
            const startSoc = Math.max(25, 25 + (0, utils_1.randomInt)(15));
            const endSoc = 95;
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'COMPLETED');`, [battery.stationId, battery.id, startTime, endTime, startSoc, endSoc, 95, energyAddedKwh]);
        }
        ...
        for (const truck of trucksToPlace) {
            ...
            const station = await (0, connection_1.getQuery)("SELECT name, locationLat, locationLng FROM stations WHERE id = ?;", [stationId]);
            // Use station coordinates or fallback
            const coordinate = (0, utils_1.getStationCoordinate)({ id: stationId, name: station?.name ?? "" });
            await (0, connection_1.runQuery)(`UPDATE trucks 
         SET currentStationId = ?, status = 'READY', locationLat = ?, locationLng = ?
         WHERE id = ?;`, [stationId, coordinate.lat, coordinate.lng, truck.id]);
        }
    }
}

...
exports.runFreightPhase = runFreightPhase;
const connection_1 = require("../../../database/connection");
const utils_1 = require("../utils");
async function runFreightPhase(context) {
...
async function generateShipments(timestamp, context) {
    const activeCount = await (0, connection_1.getQuery)("SELECT COUNT(*) as count FROM shipments WHERE status IN ('ASSIGNED','IN_TRANSIT');");
    ...
    if ((activeCount?.count ?? 0) < targetActive) {
        ...
        for (let index = 0; index < toCreate; index += 1) {
            const pickupIdx = (0, utils_1.randomInt)(stationCoords.length - 1);
            const deliveryIdx = (pickupIdx + 1 + (0, utils_1.randomInt)(3)) % stationCoords.length;
      ...
      `, [
            ...
            await (0, connection_1.runQuery)("UPDATE drivers SET status = 'ACTIVE' WHERE id = ?;", [selectedDriver.id]);
            // Create shipment events
            await (0, connection_1.runQuery)("INSERT INTO shipment_events (shipmentId, eventType, message, timestamp) VALUES (?, 'REQUESTED', 'Shipment request created by simulation', ?);", [result.lastID, timestamp]);
            await (0, connection_1.runQuery)("INSERT INTO shipment_events (shipmentId, eventType, message, timestamp) VALUES (?, 'ASSIGNED', 'Shipment assigned by simulation', ?);", [result.lastID, timestamp]);
        }
    }
}
async function transitionToInTransit(timestamp, context) {
    ...
    const toTransit = await (0, connection_1.allQuery)(`SELECT id, status, truckId, driverId FROM shipments WHERE status = 'ASSIGNED' ORDER BY id ASC LIMIT ${limit};`);
    for (const shipment of toTransit) {
        await (0, connection_1.runQuery)("UPDATE shipments SET acceptedAt = COALESCE(acceptedAt, ?), pickupConfirmedAt = COALESCE(pickupConfirmedAt, ?), status = 'IN_TRANSIT' WHERE id = ?;", [timestamp, timestamp, shipment.id]);
        await (0, connection_1.runQuery)("INSERT INTO shipment_events (shipmentId, eventType, message, timestamp) VALUES (?, 'IN_TRANSIT', 'Shipment is now in transit', ?);", [shipment.id, timestamp]);
    }
}
async function completeDeliveries(timestamp, context) {
    // Apply scenario modifier for freight completion
    const completionMultiplier = context.scenarioModifiers.freightCompletionMultiplier ?? 1.0;
    const baseLimit = 3;
    const limit = Math.floor(baseLimit * completionMultiplier);
    // Complete IN_TRANSIT shipments per cycle
    const toDeliver = await (0, connection_1.allQuery)(`SELECT id, status, truckId, driverId FROM shipments WHERE status = 'IN_TRANSIT' ORDER BY id ASC LIMIT ${limit};`);
    for (const shipment of toDeliver) {
        await (0, connection_1.runQuery)("UPDATE shipments SET deliveryConfirmedAt = ?, status = 'DELIVERED' WHERE id = ?;", [timestamp, shipment.id]);
        await (0, connection_1.runQuery)("INSERT INTO shipment_events (shipmentId, eventType, message, timestamp) VALUES (?, 'DELIVERED', 'Shipment delivered successfully', ?);", [shipment.id, timestamp]);
        // Update driver completed trips
        if (shipment.driverId) {
            await (0, connection_1.runQuery)("UPDATE drivers SET completedTrips = completedTrips + 1 WHERE id = ?;", [
                shipment.driverId,
            ]);
        }
        // Return truck to READY status
        if (shipment.truckId) {
            await (0, connection_1.runQuery)("UPDATE trucks SET status = 'READY', availability = 'AVAILABLE' WHERE id = ?;", [
                shipment.truckId,
            ]);
        }
    }

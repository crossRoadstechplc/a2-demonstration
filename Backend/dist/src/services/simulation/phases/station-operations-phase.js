...
exports.runStationOperationsPhase = runStationOperationsPhase;
const connection_1 = require("../../../database/connection");
const utils_1 = require("../utils");
async function runStationOperationsPhase(context) {
    const { timestamp } = context;
    // Get all trucks that have arrived at stations (progress >= 1)
    const trucks = await (0, connection_1.allQuery)(`
    ...
    for (const truck of trucks) {
        ...
        const outgoingBattery = await (0, connection_1.getQuery)(`
...
async function trySwapForTruck(truck, stationId, outgoingBattery, arrivalSoc, timestamp, context) {
  ...
  `, [stationId]);
    ...
    await (0, connection_1.runQuery)("UPDATE batteries SET truckId = NULL, stationId = ?, soc = ?, status = 'CHARGING', cycleCount = cycleCount + 1 WHERE id = ?;", [stationId, clampedArrivalSoc, outgoingBattery.id]);
    ...
    const energyDeliveredKwh = (0, utils_1.round2)(Math.max(0, (incomingBattery.capacityKwh * (incomingBattery.soc - arrivalSoc)) / 100 + extraEnergy));

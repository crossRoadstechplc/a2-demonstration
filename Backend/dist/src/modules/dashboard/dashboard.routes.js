...
dashboardRouter.get("/dashboard/a2", async (_req, res, next) => {
    try {
        const activeTrucks = await count("SELECT COUNT(*) as count FROM trucks WHERE status = 'IN_TRANSIT';");
        const swapsToday = await count("SELECT COUNT(*) as count FROM swap_transactions WHERE date(timestamp) = date('now');");
        const batteriesReady = await count("SELECT COUNT(*) as count FROM batteries WHERE status = 'READY';");
        const energyToday = await sum("SELECT COALESCE(SUM(energyDeliveredKwh), 0) as total FROM swap_transactions WHERE date(timestamp) = date('now');");
        const incidents = await count(`
      SELECT COUNT(*) as count
      FROM driver_telemetry
      WHERE speed > 100 OR brakeForce > 0.8;
    `);
        const stationsOnline = await count("SELECT COUNT(*) as count FROM stations WHERE status = 'ACTIVE';");
        res.status(200).json({
            activeTrucks,
            swapsToday,
            batteriesReady,
...
dashboardRouter.get("/dashboard/station/:id", async (req, res, next) => {
    try {
        ...
        const batteriesAtStation = await count("SELECT COUNT(*) as count FROM batteries WHERE stationId = ?;", [stationId]);
        ...
        const swapsToday = await count("SELECT COUNT(*) as count FROM swap_transactions WHERE stationId = ? AND date(timestamp) = date('now');", [stationId]);
        const energyToday = await sum("SELECT COALESCE(SUM(energyDeliveredKwh), 0) as total FROM swap_transactions WHERE stationId = ? AND date(timestamp) = date('now');", [stationId]);
        res.status(200).json({
...
dashboardRouter.get("/dashboard/fleet/:id", async (req, res, next) => {
    try {
        ...
        const activeTrucks = await count("SELECT COUNT(*) as count FROM trucks WHERE fleetId = ? AND status = 'IN_TRANSIT';", [fleetId]);
...
dashboardRouter.get("/dashboard/eeu", async (_req, res, next) => {
    try {
        const swapsToday = await count("SELECT COUNT(*) as count FROM swap_transactions WHERE date(timestamp) = date('now');");
        const energySoldToday = await sum("SELECT COALESCE(SUM(energyKwh), 0) as total FROM receipts WHERE date(timestamp) = date('now');");
        const revenueToday = await sum("SELECT COALESCE(SUM(energyCharge), 0) as total FROM receipts WHERE date(timestamp) = date('now');");
        const vatShareToday = await sum("SELECT COALESCE(SUM(vat / 2), 0) as total FROM receipts WHERE date(timestamp) = date('now');");
...
dashboardRouter.get("/dashboard/freight/:customerId", async (req, res, next) => {
    try {
        ...
        const totalShipments = await count("SELECT COUNT(*) as count FROM shipments WHERE customerId = ?;", [customerId]);
        const activeShipments = await count("SELECT COUNT(*) as count FROM shipments WHERE customerId = ? AND status IN ('REQUESTED', 'ASSIGNED', 'IN_TRANSIT');", [customerId]);
    ...
    `, [customerId]);
      ...
      LIMIT 10;
    `, [customerId]);
        const deliveryConfirmations = await count("SELECT COUNT(*) as count FROM shipments WHERE customerId = ? AND deliveryConfirmedAt IS NOT NULL;", [customerId]);
        res.status(200).json({
            customerId,
            totalShipments,
            activeShipments,

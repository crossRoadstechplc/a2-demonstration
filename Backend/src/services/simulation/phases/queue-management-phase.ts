...
import type { SimulationContext, TruckRow } from "../types";
import { getStationCoordinate, round2 } from "../utils";

export async function runQueueManagementPhase(context: SimulationContext): Promise<void> {
  ...
  const trucksNeedingSwap = await allQuery<TruckRow>(
    `
  ...
  for (const truck of trucksNeedingSwap) {
    ...
    for (const station of stations) {
      const stationCoord = getStationCoordinate(station);
    ...
    const existingQueue = await getQuery<{ id: number; distanceKm: number }>(
      "SELECT id, distanceKm FROM swap_queue WHERE truckId = ? AND stationId = ? AND status IN ('PENDING', 'ARRIVED');",
      [truck.id, nearestStation.id]
    );
    ...
    if (!existingQueue) {
      ...
      await runQuery(
        ...
        VALUES (?, ?, ?, ?, ?, 'PENDING');
      `,
        [truck.id, nearestStation.id, timestamp, estimatedArrival, round2(minDistance)]
      );
    } else {
      ...
      await runQuery(
        "UPDATE swap_queue SET distanceKm = ?, estimatedArrival = ? WHERE id = ?;",
        [round2(minDistance), estimatedArrival, existingQueue.id]
      );
    }
  }
  ...
  const arrivedTrucks = await allQuery<{ truckId: number; stationId: number }>(
    `
  ...
  for (const arrival of arrivedTrucks) {
    await runQuery(
      "UPDATE swap_queue SET status = 'ARRIVED' WHERE truckId = ? AND stationId = ? AND status = 'PENDING';",
      [arrival.truckId, arrival.stationId]
    );
  }

  // Mark queue entries as COMPLETED when swap is done
  const completedSwaps = await allQuery<{ truckId: number; stationId: number }>(
    `
  ...
  for (const swap of completedSwaps) {
    await runQuery(
      "UPDATE swap_queue SET status = 'COMPLETED' WHERE truckId = ? AND stationId = ? AND status IN ('PENDING', 'ARRIVED');",
      [swap.truckId, swap.stationId]
    );
  }

  // Clean up old completed queue entries (older than 1 day)
  await runQuery(
    "DELETE FROM swap_queue WHERE status = 'COMPLETED' AND bookedAt < datetime('now', '-1 day');"
  );
}

async function reorderQueueByDistance(): Promise<void> {

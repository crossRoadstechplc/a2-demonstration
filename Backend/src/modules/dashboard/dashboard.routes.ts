...
import { scopedQueries } from "../../services/scoped-queries";
...
dashboardRouter.get(
  ...
  async (_req, res, next) => {
    try {
      ...
      const activeTrucks = await scopedQueries.a2.countActiveTrucks();
      const swapsToday = await scopedQueries.a2.countSwapsToday();
      const batteriesReady = await scopedQueries.a2.countBatteriesReady();
      const chargingActive = await scopedQueries.a2.countChargingActive();
      const corridorEnergyToday = await scopedQueries.a2.sumCorridorEnergyToday();
      const corridorRevenue = await scopedQueries.a2.sumCorridorRevenue();
      const a2Share = await scopedQueries.a2.sumA2Share();
      const eeuShare = await scopedQueries.a2.sumEeuShare();
      const vatCollected = await scopedQueries.a2.sumVatCollected();
      const stationsOnline = await scopedQueries.a2.countStationsOnline();
...
dashboardRouter.get(
  "/dashboard/a2/system-health",
  requireAuth,
  requireAnyRole(["ADMIN", "A2_OPERATOR"]),
  async (_req, res, next) => {
    try {
      const stationsOnline = await scopedQueries.a2.countStationsOnline();
      const stationsOffline = await count(
        "SELECT COUNT(*) as count FROM stations WHERE status = 'INACTIVE';"
      );
      const trucksActive = await scopedQueries.a2.countActiveTrucks();
      const trucksIdle = await count(
        "SELECT COUNT(*) as count FROM trucks WHERE status = 'IDLE';"
      );
      const trucksMaintenance = await count(
        "SELECT COUNT(*) as count FROM trucks WHERE status = 'MAINTENANCE';"
      );
      const driversActive = await count(
        "SELECT COUNT(*) as count FROM drivers WHERE status IN ('ACTIVE', 'ON_DUTY');"
      );
      const driversInactive = await count(
        "SELECT COUNT(*) as count FROM drivers WHERE status IN ('AVAILABLE', 'RESTING');"
      );
...
dashboardRouter.get(
  ...
  async (_req, res, next) => {
    try {
      const stations = await allQuery<{ id: number; name: string; capacity: number }>(
        "SELECT id, name, capacity FROM stations WHERE status = 'ACTIVE' ORDER BY id;"
      );

      const swapsByStation = await allQuery<{ stationId: number; count: number }>(
        `SELECT stationId, COUNT(*) as count FROM swap_transactions
         WHERE date(timestamp, 'localtime') = date('now', 'localtime')
         GROUP BY stationId;`
      );
      const swapMap = new Map(
        (swapsByStation as Array<{ stationId: number; count: number }>).map((r) => [r.stationId, r.count])
      );
      ...
      const batteryByStation = await allQuery<{
        stationId: number;
        ready: number;
        charging: number;
        total: number;
      ...
      const batteryRows = batteryByStation as Array<{
        stationId: number;
        ready: number;
        charging: number;
        total: number;
      ...
      const chargingByStation = await allQuery<{ stationId: number; activeSessions: number }>(
        `SELECT stationId, COUNT(*) as activeSessions FROM charging_sessions
         WHERE status = 'ACTIVE' GROUP BY stationId;`
      );
      const chargingMap = new Map(
        (chargingByStation as Array<{ stationId: number; activeSessions: number }>).map((r) => [
          r.stationId,
          r.activeSessions,
        ])
      );
      ...
      const truckMovementRows = await allQuery<{ status: string; count: number }>(
        `SELECT status, COUNT(*) as count FROM trucks GROUP BY status;`
      );
      const truckMovement = (truckMovementRows as Array<{ status: string; count: number }>).map(
        (r) => ({ label: r.status || "UNKNOWN", value: r.count })
      );
...
dashboardRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const batteriesAtStation = await scopedQueries.station.countTotalBatteries(req);
      const activeChargingSessions = await count(
        "SELECT COUNT(*) as count FROM charging_sessions WHERE stationId = ? AND status = 'ACTIVE';",
        [stationId]
      );
      const swapsToday = await scopedQueries.station.countSwapsToday(req);
      const energyToday = await scopedQueries.station.sumEnergyConsumedToday(req);
    
      // Revenue tracking
      const revenueTodayEtb = await scopedQueries.station.sumRevenueToday(req);
      const revenueThisMonthEtb = await scopedQueries.station.sumRevenueThisMonth(req);

      // Energy tracking
      const energyChargingNowKwh = await scopedQueries.station.sumEnergyChargingNow(req);
    ...
    const batteryCount = await count(
      "SELECT COUNT(*) as count FROM batteries WHERE stationId = ?;",
      [stationId]
    );
    const stationData = await getQuery<{ capacity: number }>(
      "SELECT capacity FROM stations WHERE id = ?;",
      [stationId]
    );
    ...
    const chargingSessions = await allQuery<{
      ...
      `SELECT id, batteryId, currentSoc, targetSoc, energyAddedKwh
       FROM charging_sessions
       WHERE stationId = ? AND status = 'ACTIVE'
       ORDER BY id ASC
       LIMIT ?;`,
      [stationId, realisticChargerCount]
    );
    
    const chargerStatus: Array<{
      chargerId: string;
      status: string;
      outputKw: number;
    ...
    try {
      ...
      const stationData = await getQuery<{ 
        locationLat: number | null; 
        locationLng: number | null;
        name: string;
      }>(
        "SELECT locationLat, locationLng, name FROM stations WHERE id = ?;",
        [stationId]
      );
      
      // Fallback to hardcoded coordinates if not in DB
      const stationCoordinateByName: Record<string, { lat: number; lng: number }> = {
      ...
      if (stationLat && stationLng) {
        const incomingTrucks = await allQuery<{
          id: number;
          ...
          `SELECT id, plateNumber, locationLat, locationLng, currentSoc, status
           FROM trucks
      ...
      const totalQueueSize = await scopedQueries.station.countQueueSize(req);

      // Additional aggregates
      const readyBatteries = await scopedQueries.station.countReadyBatteries(req);
      const chargingBatteries = await scopedQueries.station.countChargingBatteries(req);
      ...
      const batteryInventoryByStatus = {
        ...
        CHARGING: await count(
          "SELECT COUNT(*) as count FROM batteries WHERE stationId = ? AND status = 'CHARGING';",
          [stationId]
        ),
        MAINTENANCE: await count(
          "SELECT COUNT(*) as count FROM batteries WHERE stationId = ? AND status = 'MAINTENANCE';",
          [stationId]
        ),
        IN_TRUCK: await count(
          "SELECT COUNT(*) as count FROM batteries WHERE stationId = ? AND status = 'IN_TRUCK';",
          [stationId]
        ),
      };

      // Batteries ready for swap (SOC >= 80%)
      const batteriesReadyForSwap = await allQuery<{
        id: number;
        soc: number;
        health: number;
        cycleCount: number;
      }>(
        `SELECT id, soc, health, cycleCount
         FROM batteries
      ...
      const activeChargingSessionsList = await allQuery<{
        id: number;
        batteryId: number;
        startTime: string;
        ...
        `SELECT id, batteryId, startTime, startSoc, currentSoc, targetSoc, energyAddedKwh
         FROM charging_sessions
         WHERE stationId = ? AND status = 'ACTIVE'
         ORDER BY id ASC;`,
        [stationId]
      ...
      const recentCompletedChargingSessions = await allQuery<{
        id: number;
        batteryId: number;
        startTime: string;
        endTime: string | null;
        ...
        `SELECT id, batteryId, startTime, endTime, energyAddedKwh
         FROM charging_sessions
      ...
      const recentSwaps = await allQuery<{
        ...
        `SELECT id, truckId, incomingBatteryId, outgoingBatteryId, arrivalSoc, energyDeliveredKwh, timestamp
      ...
      const trucksCurrentlyAtStation = await allQuery<{
        id: number;
        plateNumber: string | null;
        currentSoc: number;
        status: string;
      ...
      // Charger faults open
      const chargerFaultsOpen = await count(
        `SELECT COUNT(*) as count
         FROM charger_faults
         WHERE stationId = ? AND status = 'OPEN';`,
        [stationId]
      );
      ...
      res.status(200).json({
        ...
        trucksAtStationCount: await (async () => {
          ...
          const criticalApproaching = await count(
            `SELECT COUNT(*) as count FROM trucks WHERE status = 'IN_TRANSIT' AND currentSoc < 35;`
          );
...
dashboardRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const availableTrucks = await scopedQueries.fleet.countAvailableTrucks(req);
      const activeDrivers = await scopedQueries.fleet.countActiveDrivers(req);
      const swapsToday = await scopedQueries.fleet.countSwapsToday(req);
      const fleetEnergyCostEtb = await scopedQueries.fleet.sumFleetEnergyCost(req);
      const completedTrips = await scopedQueries.fleet.countCompletedTrips(req);
      const maintenanceAlerts = await scopedQueries.fleet.countMaintenanceAlerts(req);
      const refrigeratedTrucksActive = await scopedQueries.fleet.countRefrigeratedActive(req);
...
dashboardRouter.get(
  "/dashboard/fleet/:id/energy-by-truck",
  requireAuth,
  requireAnyRole(["FLEET_OWNER", "ADMIN", "A2_OPERATOR"]),
  async (req, res, next) => {
    try {
      ...
      const rows = await allQuery<{
        ...
        `SELECT t.id as truckId, t.plateNumber,
      ...
      const energyByTruck = (rows as Array<{ truckId: number; plateNumber: string | null; energyKwh: number }>).map(
        (r) => ({
          truckId: r.truckId,
  ...
  async (req, res, next) => {
    try {
      ...
      const trucks = await allQuery<{
        ...
        "SELECT id, plateNumber, status, currentSoc, truckType FROM trucks WHERE fleetId = ? ORDER BY id;",
        [fleetId]
      );
      const energyRows = await allQuery<{ truckId: number; energyKwh: number }>(
        `SELECT t.id as truckId, COALESCE(SUM(s.energyDeliveredKwh), 0) as energyKwh
         FROM trucks t
         LEFT JOIN swap_transactions s ON s.truckId = t.id AND date(s.timestamp, 'localtime') = date('now', 'localtime')
         WHERE t.fleetId = ?
         GROUP BY t.id;`,
      ...
      const energyByTruck = new Map(
        (energyRows as Array<{ truckId: number; energyKwh: number }>).map((r) => [
          r.truckId,
      ...
      for (const t of trucks as Array<{ id: number; plateNumber: string | null; status: string; currentSoc: number; truckType: string | null }>) {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="fleet-${fleetId}-report-${new Date().toISOString().slice(0, 10)}.csv"`
      );
...
dashboardRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const totalNetworkLoad = await scopedQueries.eeu.countTotalNetworkLoad();
      const stationEnergy = await scopedQueries.eeu.sumStationEnergy(timeframe);
      const electricityDelivered = await scopedQueries.eeu.sumElectricityDelivered(timeframe);
      const eeuRevenueShare = await scopedQueries.eeu.sumEeuRevenueShare(timeframe);
      const activeChargingSessions = await scopedQueries.eeu.countActiveChargingSessions();
      ...
      const stationChargerCounts = await allQuery<{
        stationId: number;
        activeChargers: number;
      }>(
        `SELECT stationId, COUNT(*) as activeChargers 
       FROM charging_sessions 
       WHERE status = 'ACTIVE' 
       GROUP BY stationId;`
      );
      ...
      // Get station loads
      const stationLoads = await scopedQueries.eeu.getStationLoads();

      // Get peak load station
      const peakLoadStation = await scopedQueries.eeu.getPeakLoadStation();

      // Generate 24-hour forecast
      const forecast24h = await scopedQueries.eeu.generate24HourForecast();
...
dashboardRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const totalShipments = await scopedQueries.freight.countTotalShipments(req, timeframe);
      const activeShipments = await scopedQueries.freight.countActiveShipments(req, timeframe);
      const deliveredShipments = await scopedQueries.freight.countDeliveredShipments(req, timeframe);
    ...
    `,
      ...
      const deliveryConfirmations = await count(
        `SELECT COUNT(*) as count FROM shipments WHERE customerId = ? AND deliveryConfirmedAt IS NOT NULL ${dateFilterForShipments};`,
        [customerId]
      );
